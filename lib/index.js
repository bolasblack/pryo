...
const getGeneratorRootDir = () => {
  ...
  const generatorRootDir = path.resolve(pkgFilePath, '../', pkgFileContent['generator-folder'] || 'generators')

  if (!isFolderExist(generatorRootDir)) {
    throw new errors.GeneratorFolderNotExistError(`Generator folder ${generatorRootDir} not exists`)
  }
...
module.exports = {
  ...
  run: (generatorName, argv, options, done) => {
    ...
    if (env.getGeneratorsMeta()[generatorName]) {
      env.run([`_currentApp:${generatorName}`].concat(argv), options, done)
    } else {
      env.lookup(() => {
