...
describe('index', () => {
  ...
  describe('.getGeneratorRootDir', () => {
    ...
    it('support `generator-folder` field in package.json', () => {
      mockFS({
        '/test/path/to/package.json': JSON.stringify({
          'generator-folder': 'hello',
        }),
        '/test/path/to/hello': {},
      })
      process.chdir('/test/path/to/')
    ...
    it('throw error if package.json not exist', () => {
      ...
      process.chdir('/test/path/to/')
      expect(() => pryo.getGeneratorRootDir()).toThrowError(
        pryo.ProjectRootNotFoundError,
      )
    })
    ...
    it('throw error if package.json is invalid', () => {
      ...
      process.chdir('/test/path/to/')
      expect(() => pryo.getGeneratorRootDir()).toThrowError(
        pryo.PackageFileParseFailedError,
      )
    })
    ...
    it('throw error if generator folder not exist', () => {
      ...
      process.chdir('/test/path/to/')
      expect(() => pryo.getGeneratorRootDir()).toThrowError(
        pryo.GeneratorFolderNotExistError,
      )
    })
  })
  ...
  describe('.matchGeneratorName', () => {
    it('works', () => {
      ...
      expect(pryo.matchGeneratorName('not exist generator')).toBe(
        'not exist generator',
      )
  ...
  describe('.createEnv', () => {
    it('works', () => {
      ...
      return new Promise((resolve, reject) => {
        process.chdir('/test/path/to/')
        const env = pryo.createEnv([
          {
            name: 'hello',
            Generator,
            fullPath: '/test/path/to/generators/hello',
          },
        ])
        env.run('_currentApp:hello', {}, () => {
          expect(generatorTestResults.templatePath).toBe(
            '/test/path/to/generators/hello/templates',
          )
    ...
    it('wont update `.resolved` property if Generator not in generator-folder', () => {
      mockFS({
        '/test/path/to': {
          'package.json': '{"name": "hello"}',
          generators: {},
      ...
      const env = pryo.createEnv([])
      env.registerStub(Generator, 'vendor-generator')
      return new Promise((resolve, reject) => {
        env.run('vendor-generator', {}, () => {
          expect(generatorTestResults.templatePath).toBe(
            path.resolve(__dirname, '../../templates'),
          )
          expect(generatorTestResults.destinationPath).toBe(
            path.resolve('/test/path/to'),
          )
          resolve()
        })
      })
