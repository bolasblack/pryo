...
describe('index', () => {
  ...
  describe('.matchGeneratorName', () => {
    it('works', () => {
      ...
      expect(pryo.matchGeneratorName('not exist generator')).toBe('not exist generator')
    ...
    it('wont update `.resolved` property if Generator not in generator-folder', () => {
      ...
      return new Promise((resolve, reject) => {
        env.run('vendor-generator', {}, () => {
          expect(generatorTestResults.templatePath).toBe(path.resolve(__dirname, '../../templates'))
          expect(generatorTestResults.destinationPath).toBe(path.resolve('/test/path/to'))
          resolve()
        })
      })
    })
